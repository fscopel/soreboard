...
export const VideoModule = () => {
  ...
  const [visionBackend, setVisionBackend] = useState<"webgl" | "cpu" | null>(
    null
  );
...
const IframeLoadHint = () => {
  ...
  return (
    <div className="absolute top-2 left-2 right-2 text-xs text-yellow-200 bg-black/50 px-3 py-2 rounded">
      If the feed does not appear, it may be blocked from embedding
      (X-Frame-Options) or require authentication. Provide a direct stream URL
      (e.g., .m3u8) for reliable playback.
    </div>
  );
};
